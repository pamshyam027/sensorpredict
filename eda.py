# -*- coding: utf-8 -*-
"""
Created on Wed Mar 24 09:38:33 2021
...
#Loading the data and convert timesetamps

df =  pd.read_csv('C:/Users/Bharath/Desktop/Honeywell/Week 2 and 3/sensor.csv')
...
# check the sensors that have more than 2% of nans - lets see it together with the machine status
...
# Correlation check if some of the sensors are correlated


corr = df[signal_columns].corr()
sns.heatmap(corr)

#We can see strongly correlated group of sensors - from sensor_14 to sensor_26. There also some other correlated groups but not as strong as the mentioned one.

# Failure data investigation - locate the failure events
...
# We can see that first 6 failures happened during late evening/night - only 7th was at 2 pm Lets check the sensors behaviour before the failure and during the recovering state
